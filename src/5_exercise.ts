...
export async function run() {
  ...
  await Mina.transaction(account1, async () => {
    ...
    await snappInstance.spend(
      amount,
      SignatureWithSigner.create(privateKeys[4], snappNonce.toFields())
    );
    // Send it to account 2
    Party.createUnsigned(account1.toPublicKey()).balance.addInPlace(amount);
  })
    .send()
    .wait();
